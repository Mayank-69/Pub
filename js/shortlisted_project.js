...
(function(){
    ...
    function savediv()
    {
      firebase.firestore().collection("User shortlisted").doc(getEmail).collection("Selected user list").where("email","==",getEmail).onSnapshot(function(querySnapshot) {
        var cities1 = [];
        
        // var titleyo =[];
            querySnapshot.forEach(function(doc) {
          
            cities1.push(doc.data().div);
        
                document.getElementById("autodiv").innerHTML=cities1.join(" ");
